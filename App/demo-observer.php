...
use App\Processes\ObserveAndOffloadGateProcess;
...
$docs = <<<TEXT
Read "demo-observer.md" file before running this demo.
TEXT;
echo "\n{$docs}\n\n";
...
$app->setProcessRegistry(
        (new ProcessRegistry())
                ->add(ObserveAndOffloadGateProcess::class)
                ->add(ParallelProcess_A::class)
                ->add(ParallelProcess_B::class)
                ->add(ParallelProcess_C::class)
);

$return = $app->process(ObserveAndOffloadGateProcess::class, null);
